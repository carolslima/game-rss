...
export default async function fetchPosts(config, context) {
  ...
  const postsBasePath = isAuthenticated
    ? `${PATREON_URL}/api/oauth2/v2/campaigns/${campaignId}/posts?include=media&fields%5Bpost%5D=title,content,url,published_at,is_paid&fields%5Bmedia%5D=image_urls`
  ...
  while (postsUrl && pageCount < maxPages) {
    ...
    try {
      ...
      if (body?.errors && pageCount === 1) {
        log("[DEBUG]", `API errors: ${JSON.stringify(body.errors).substring(0, 500)}`);
      }
    } catch (err) {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      if (pageCount === 1 && isAuthenticated) {
        const first = data[0];
        log("[DEBUG]", `Estrutura do 1º post: id=${first.id}, type=${first.type}`);
        log("[DEBUG]", `  attributes keys: ${Object.keys(first.attributes || {}).join(", ")}`);
        log("[DEBUG]", `  title: "${(first.attributes?.title || "").substring(0, 60)}"`);
        log("[DEBUG]", `  content: ${first.attributes?.content ? 'presente (' + first.attributes.content.length + ' chars)' : 'AUSENTE'}`);
        log("[DEBUG]", `  content_json_string: ${first.attributes?.content_json_string ? 'presente (' + first.attributes.content_json_string.length + ' chars)' : 'AUSENTE'}`);
        log("[DEBUG]", `  url: ${first.attributes?.url}`);
        log("[DEBUG]", `  published_at: ${first.attributes?.published_at}`);
        log("[DEBUG]", `  post_type: ${first.attributes?.post_type}`);
        log("[DEBUG]", `  is_paid: ${first.attributes?.is_paid}`);
        log("[DEBUG]", `  teaser_text: ${first.attributes?.teaser_text ? 'presente' : 'AUSENTE'}`);
        log("[DEBUG]", `  image: ${first.attributes?.image ? JSON.stringify(first.attributes.image).substring(0, 100) : 'AUSENTE'}`);

        const withContent = data.filter(p => p.attributes?.content || p.attributes?.content_json_string).length;
        log("[DEBUG]", `  ${withContent}/${data.length} posts com conteúdo`);

        if (body?.meta) {
          log("[DEBUG]", `  meta: ${JSON.stringify(body.meta)}`);
        }
      }

      // Build media map from included (v2 only)
      let mediaMap = {};
      if (isAuthenticated && Array.isArray(body?.included)) {
      ...
      const normalized = data.map((post) =>
        normalizePost(post, campaignWebUrl, mediaMap)
      );
    ...
    if (body?.links?.next) {
      ...
      if (isAuthenticated && !postsUrl.includes("fields%5Bpost%5D")) {
        postsUrl += "&include=media&fields%5Bpost%5D=title,content,url,published_at,is_paid&fields%5Bmedia%5D=image_urls";
      }
    } else {
      postsUrl = null;
    }
