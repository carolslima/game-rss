...
export default async function fetchPosts(config, context) {
  ...
  const postsBasePath = isAuthenticated
    ? `${PATREON_URL}/api/oauth2/v2/campaigns/${campaignId}/posts?fields%5Bpost%5D=title,content,content_json_string,url,published_at,post_type,image,teaser_text`
  ...
  while (postsUrl && pageCount < maxPages) {
    ...
    if (body?.links?.next) {
      ...
      if (isAuthenticated && !postsUrl.includes("fields%5Bpost%5D")) {
        postsUrl += "&fields%5Bpost%5D=title,content,content_json_string,url,published_at,post_type,image,teaser_text";
      }
    } else {
      postsUrl = null;
    }
