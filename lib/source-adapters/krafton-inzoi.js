...
export default async function fetchPosts(config, context) {
  ...
  const API_BASE = opts.apiBase || "https://api-foc.krafton.com/content/post/news";
  ...
  let namespace = context.env.KRAFTON_NAMESPACE || opts.namespaceFallback || "";
  let game = context.env.KRAFTON_GAME || opts.gameFallback || "";

  if (!context.env.KRAFTON_NAMESPACE || !context.env.KRAFTON_GAME) {
    try {
      log("[INFO]", "Auto-descobrindo configuração da API...");
      const discoveryUrl = opts.discoveryUrl || "https://playinzoi.com/pt-br/news";
      ...
      if (resp.ok) {
        ...
        if (!context.env.KRAFTON_NAMESPACE && nsMatch) namespace = nsMatch[1];
        if (!context.env.KRAFTON_GAME && gameMatch) game = gameMatch[1];
      ...
      log("[WARN]", `Auto-descoberta falhou: ${err.message} — usando fallback`);
    }
  }
  ...
  function getApiHeaders() {
    return {
      ...
      "User-Agent": "GameRSS-Agent/1.0",
      Origin: opts.apiHeaders?.Origin || "https://playinzoi.com",
      Referer: opts.apiHeaders?.Referer || "https://playinzoi.com/pt-br/news",
      "service-lang": "pt-br",
  ...
  // --- Normalização ---
  const urlTemplate = opts.urlTemplate || "https://playinzoi.com/{lang}/news/{postId}";
